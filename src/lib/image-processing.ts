...
export const memeImage = (file: File, topText: string, bottomText: string): Promise<Blob> => {
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.src = URL.createObjectURL(file);
    img.onload = () => {
      ...
      if (bottomText) {
        ...
        ctx.strokeText(bottomText.toUpperCase(), canvas.width / 2, canvas.height - 10 - fontSize);
        ctx.fillText(bottomText.toUpperCase(), canvas.width / 2, canvas.height - 10 - fontSize);
      }
