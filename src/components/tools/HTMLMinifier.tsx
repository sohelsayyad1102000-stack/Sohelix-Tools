...
export const HTMLMinifier: React.FC = () => {
  ...
  const minifyHTML = () => {
    ...
    if (options.collapseBooleanAttributes) {
      const boolAttrs = ['checked', 'disabled', 'readonly', 'required', 'multiple', 'selected', 'autofocus', 'autoplay', 'controls', 'loop', 'muted', 'hidden'];
      boolAttrs.forEach(attr => {
        const re = new RegExp(`${attr}="[^"]*"`, 'gi');
        minified = minified.replace(re, attr);
      });
    }
        ...
        <div className="flex flex-wrap items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Fine-tune the output</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">Source HTML</h3>
            <button 
              onClick={() => setInput('')}
        ...
        <div className="space-y-4">
          ...
          <div className="relative h-[348px] rounded-3xl border border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-gray-950 overflow-hidden shadow-inner">
            <pre className="h-full p-6 text-xs font-mono text-blue-600 dark:text-blue-400 overflow-auto whitespace-pre-wrap break-all">
              {output || <span className="text-gray-400 italic">Minified HTML will appear here...</span>}
            </pre>
          </div>
        </div>
      </div>
